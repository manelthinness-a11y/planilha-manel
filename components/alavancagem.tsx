...
import {leverageEntries,nextLeverage,resetStakeFor,type Group} from '@/lib/alavancagem';
...
function LeverageRegister({rows,disabled,onUpdated,group}:PanelProps&{group:Group}){
 ...
 return <div>
  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,flexWrap:'wrap'}}>
   <p className="hint" style={{margin:0}}>{group==='individual'?'Registros Individual':'Registros Alavancagem 1,3'} · Histórico e sequência de stakes independentes.{group==='alavancagem'&&' Cada entrada registrada aqui gera automaticamente uma entrada espelhada em Individual, com a stake padrão configurada.'}</p>
   {group==='individual'&&<button type="button" className="text-button" style={{display:'inline-flex',alignItems:'center',gap:4,whiteSpace:'nowrap'}} disabled={disabled||busy} onClick={()=>{setError('');setStakeInput((defaultStake/100).toFixed(2));setSettingsOpen(true);}}><Settings size={16}/>Stake padrão: {money(defaultStake)}</button>}
  </div>
  ...
  <Dialog open={settingsOpen} onOpenChange={v=>{if(!busy)setSettingsOpen(v);}}><DialogContent><DialogHeader><DialogTitle>Stake padrão da Individual</DialogTitle><DialogDescription>Valor usado sempre que o ciclo reinicia (Red ou meta de R$ 50,00 atingida) e em toda entrada espelhada automaticamente a partir da Alavancagem 1,3.</DialogDescription></DialogHeader><form onSubmit={e=>{e.preventDefault();const cents=Math.round(Number(stakeInput.replace(',','.'))*100);if(!Number.isFinite(cents)||cents<100){setError('Informe um valor válido (mínimo R$ 1,00).');return;}void send({action:'set-default-stake',value:cents});}}>
