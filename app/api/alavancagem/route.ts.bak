...
const input=z.discriminatedUnion('action',[
 z.object({action:z.literal('create'),group:z.enum(['alavancagem','individual']).default('alavancagem'),event:z.string().trim().min(1).max(200),date:z.string().regex(/^\d{4}-\d{2}-\d{2}$/),expectedSequence:z.number().int().positive()}),
...
export async function POST(req:Request){try{
 ...
 if(body.action==='create'){
  ...
  const data={...next,group:body.group,event:body.event,date:body.date,result:'Pendente',prize:0};
