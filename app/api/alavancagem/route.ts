...
import {nextLeverage,settleLeverage,individualDefaultStake,individualDefaultStakeId} from '@/lib/alavancagem';
...
export async function POST(req:Request){try{
 ...
 if(body.action==='create'){
  ...
  if(mirrorNext){
   ...
   const mirrorData={...mirrorNext,stake:individualDefaultStake(rowsAfter),group:'individual',event:body.event,date:body.date,odd:body.odd,market:body.market,account:body.account,house:body.house,result:'Pendente',prize:0};
  ...
  result=row
   ...
   :await database().prepare(insertRecordSql).bind(individualDefaultStakeId,'alavancagem_setting',JSON.stringify(data),snapshot).run();
 }
 if(!result.meta.changes)return Response.json({error:'Os dados mudaram. Sincronize e confira antes de repetir.'},{status:409});
 return Response.json({ok:true},{headers:{'Cache-Control':'no-store'}});
