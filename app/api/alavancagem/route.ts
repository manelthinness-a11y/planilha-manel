...
export async function POST(req:Request){try{
 ...
 let result;
 let logEvent:{kind:string;action:string;id?:string;revision?:number;summary?:string;data?:unknown}|null=null;
 if(body.action==='create'){
  ...
  await backupLog({kind:'alavancagem',action:'create',id,revision:1,summary:body.group+' · entrada '+data.sequence+' · '+data.event,data});
  if(mirrorGroup&&mirrorNext){
   ...
   await backupLog({kind:'alavancagem',action:'create',id:mirrorId,revision:1,summary:mirrorGroup+' · entrada espelhada '+mirrorData.sequence+' · '+mirrorData.event,data:mirrorData});
  }
  ...
  logEvent={kind:'alavancagem',action:'settle',id:row.id,revision:body.revision+1,summary:body.group+' · '+body.result+' · '+data.event,data};
  ...
  logEvent={kind:'alavancagem',action:'delete',id:row.id,revision:body.revision,summary:body.group+' · exclusão · '+row.data.event,data:row.data};
 }else{
  ...
  logEvent={kind:'alavancagem_setting',action:row?'update':'create',id:settingId,revision:(row?.revision||0)+1,summary:'Nova stake padrão ('+body.group+'): R$ '+(body.value/100).toFixed(2).replace('.',','),data};
 }
 if(!result.meta.changes)return Response.json({error:'Os dados mudaram. Sincronize e confira antes de repetir.'},{status:409});
